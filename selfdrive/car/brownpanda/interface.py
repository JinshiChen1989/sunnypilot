...
from openpilot.selfdrive.car.brownpanda.carcontroller import CarController
from openpilot.selfdrive.car.brownpanda.carstate import CarState
from openpilot.selfdrive.car.brownpanda.values import CAR, BrownPandaFlags, CarControllerParams
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(ret, candidate, fingerprint, car_fw, experimental_long, docs):
    ...
    if ret.steerControlType == SteerControlType.torque:
      ...
      if candidate == CAR.BYD_ATTO3:
        ret.lateralTuning.torque.kp = 2.2  # Increased from 1.15 (following KIA EV6 aggressive tuning)
        ret.lateralTuning.torque.ki = 0.18  # Increased from 0.11
        ret.lateralTuning.torque.friction = 0.02  # Reduced from 0.08 (EV precision advantage)
        ret.lateralParams.torqueBP = [0., 5., 15., 30., 50.]  # Optimized breakpoints
        ret.lateralParams.torqueV = [0., 80., 160., 320., 500.]  # Enhanced torque curve
      elif candidate == CAR.BYD_DOLPHIN:
        ret.lateralTuning.torque.kp = 2.4  # Increased from 1.25 (city EV needs responsive tuning)
        ret.lateralTuning.torque.ki = 0.20  # Increased from 0.13
        ret.lateralTuning.torque.friction = 0.015  # Reduced from 0.07 (compact EV precision)
        ret.lateralParams.torqueBP = [0., 5., 12., 22., 35.]  # City-optimized breakpoints
        ret.lateralParams.torqueV = [0., 70., 140., 280., 430.]  # Enhanced for urban driving
      elif candidate == CAR.DEEPAL_S05:
