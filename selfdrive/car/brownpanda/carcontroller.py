...
from openpilot.selfdrive.car.brownpanda import brownpandacan
from openpilot.selfdrive.car.brownpanda.values import (
  CarControllerParams,
...
class CarController(CarControllerBase):
  ...
  def calculate_wheel_torque(self, desired_accel):
    ...
    wheel_torque_limited = clip(wheel_torque, min_wheel_torque, max_wheel_torque)

    # === DBC SIGNAL RANGE COMPLIANCE ===
    # Clip to DBC signal range for cmdTorque (wheel torque)
    return clip(wheel_torque_limited, -5000.0, 15475.0)
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if CC.latActive:
      ...
      # Angle rate limit (per model or default)
      if hasattr(self.params, 'ANGLE_RATE_LIMIT_UP') and self.params.ANGLE_RATE_LIMIT_UP:
        angle_rate = self.params.ANGLE_RATE_LIMIT_UP.interp(CS.out.vEgo)
      else:
        angle_rate = 30.0  # Safe fallback: 30 deg/s (conservative for all vehicles)

      # Apply angle rate limit
      limited_angle = rate_limit(desired_angle, self.last_steer_angle,
                               -angle_rate * DT_CTRL, angle_rate * DT_CTRL)
      limited_angle = clip(limited_angle, -180.0, 180.0)
      ...
      limited_torque = clip(limited_torque, -steer_max, steer_max)
